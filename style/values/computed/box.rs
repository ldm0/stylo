...
impl ToComputedValue for specified::Zoom {
    ...
    fn to_computed_value(&self, context: &Context) -> Self::ComputedValue {
        let n = match self {
            specified::Zoom::Normal => return Zoom::ONE,
            specified::Zoom::Document => return Zoom::DOCUMENT,
            specified::Zoom::Value(n) => n.0.to_number().get(),
            specified::Zoom::Calculation(node) => node
                .to_number_or_percentage_with_context(context)
                .unwrap_or(1.0)
                .max(0.0),
        };
        if n == 0.0 {
